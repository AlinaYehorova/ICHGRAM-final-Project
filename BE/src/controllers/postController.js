...
export const getUserPosts = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId, { username: 1, profile_image: 1 }).lean();
...
export const getPostById = async (req, res) => {
  ...
  try {
    ...
    const user = await User.findById(post.user_id, { _id: 0, username: 1, profile_image: 1 }).lean()
    if (!post) return res.status(404).json({ error: 'Пост не найден' });
    const postDTO = {
      ...post,
      ...user
...
export const getOtherUserPosts = async (req, res) => {
  try {
    ...
    const posts = await Post.find({ user_id: user_id }).lean(); // Поиск постов по user_id
    const user = await User.findById(user_id, {_id: 0, username: 1, profile_image: 1}).lean();
    const postsDTO = posts.map(post => ({
      ...post, 
      user_name: user.username,
      profile_image: user.profile_image
